...
double calcular_recuperacion(double recursos_actuales) {
  ...
  printf("[Evento] El porcentaje de recuperación es del: %.6f\n", porcentaje);
...
void processo_padre(pid_t pid1, pid_t pid2, int N, double lim_alto, double lim_medio, double lim_bajo) {
  ...
  for (i = 0; i < N; i++) {
      printf("* AÑO %d\n", i + 1);
      printf("[Coordinador] Los recursos disponibles para el año en curso son %.6f\n", recursos_disponibles);

      // Asigna límite de extracción según umbrales
      if (recursos_disponibles > UMBRAL_MEDIO) {
          limite = lim_alto;
          printf("[Coordinador] El límite de extracción para el año en curso es %.6f\n", lim_alto);
      ...
      printf("[Coordinador] El país 1 solicita extraer %.6f\n", solicitud_pais1);

      // Ajusta solicitud si supera recursos disponibles
      if (solicitud_pais1 > recursos_disponibles) {
          printf("[Coordinador] La solicitud de extracción del país 1 no se ha aprobado, se va a extraer el posible\n");
      ...
      printf("[Coordinador] El país 2 solicita extraer %.6f\n", solicitud_pais2);

      if (solicitud_pais2 > recursos_disponibles) {
          printf("[Coordinador] La solicitud de extracción del país 2 no se ha aprobado, se va a extraer el posible\n");
      ...
      printf("[Coordinador] Los recursos se recuperaron en %.6f unidades\n", recursos_recuperados);
  }
  ...
  //Abrimos el fichero para guardar los datos
  FILE *fp = fopen("datos_extraccion.dat", "a");
  if (fp == NULL){
    perror("Error al abrir el archivo");
    exit(1);
  }

  fprintf(fp, "%.6f %.6f %.6f %.6f\n", lim_alto, lim_medio, lim_bajo, extraccion_pais1 + extraccion_pais2);
  fclose(fp);
}
